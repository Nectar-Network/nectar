...
| Claim | Value | Date | Source |
|---|---|---|---|
| **A fresh, unconfigured borrower is discovered from events and liquidated autonomously** | Address `GDLTPYPG…` was generated at runtime, existed in no config/env/cache/evidence file, and was discovered from the incremental event sweep (`backfill=false from=4124986`), health-checked at `hf=1.4962` (matching `(100×0.42×0.75)/(20÷0.95)` exactly), then liquidated end-to-end with no config change and no restart | 2026-08-14 | `docs/evidence/d-discovery.md`; borrow tx `e31a5ee8…`, auction `4294da5b…`, atomic fill `a4af213d…` |
| **Cold start and cached restart reach the same set — INSIDE the retention window** | Live A/B on the sandbox: cold start (`backfill=true from=4004407`, 25 events) rebuilt the identical 3-address set with identical debt states and last-event ledgers; only `probed` differed | 2026-08-14 | `docs/evidence/d-discovery.md` "Direct A/B"; `TestSync_ColdAndCachedRestartAgree_WithinRetention` |
| **The cache IS load-bearing OUTSIDE that window — the "optimisation only" claim was an overclaim** | A borrower idle longer than the 120960-ledger (~7 day) window has no events left on any endpoint the keeper can reach, and interest accrual (what pushes an idle position underwater) emits no event at all. Such an address survives only because the cache remembered it. `WATCH_ADDRESSES` is the documented recovery | 2026-08-14 | `TestSync_ColdStartCannotSeeBorrowersOlderThanRetention` pins the divergence; caught by adversarial review of the D2 change |
| Debt state must come from `get_positions`, never from events | Confirmed live: after the fill the index went `debtors=1 → 0` only via a probe, then `probed=1 → 0`. No event carries a post-action liability balance | 2026-08-14 | `docs/evidence/d-discovery.md` §4; "Pool event schemas" above |
| A `fill_auction` filler becomes a borrower and is invisible to topic scanning | Decoded from event DATA[0] and indexed; the keeper's own address is excluded so it never targets itself | 2026-08-14 | `keeper/blend/index.go` `auctionFacts`/`fillerAddr`; `TestClassify_FillAuctionFindsTheFillerInTheData` |
...
| Scout at the frozen tag: 93 raw findings, 0 requiring fixes — freeze intact | cargo-scout-audit **0.3.16** (= crates.io max_version, published 2026-02-13) on both frozen contracts: vault 24 Critical/27 Medium/4 Enh, registry 3 Critical/30 Medium/5 Enh; all i128-overflow + admin-param classes FALSE-POSITIVE (adversarially upheld, zero counterexamples), rest ACCEPTED with reasons. Local-host gotcha: Homebrew `cargo` shadowing the rustup shim breaks Scout's detector build (`Failed to build detector library`); fix = `~/.cargo/bin` first in PATH | 2026-08-16 | `docs/security/SCOUT-REPORT.md` + raw output `docs/security/scout-raw/`; run log this session |
| Functional LOC at the tag: **1,071** (vault 672, registry 399); 1,438 raw | tokei 14.0.0 on the 4 production source files (code lines excl. comments/blanks) | 2026-08-16 | `docs/audit/FREEZE-NOTE.md` "LOC count" (exact command recorded) |
| Blend v2 was audited via Code4rena + Certora formal verification (Feb 24–Mar 17 2025, $125k pool) | Externally published; grounds the Audit Bank firm-preference answer. Audit Bank approved firms (per SCF handbook official rules): Certora, Code4rena, ChainSecurity, Halborn, Oak Security, OtterSec, Runtime Verification, Spearbit+Cantina, Veridise, Zellic | 2026-08-16 | code4rena.com/audits/2025-02-blend-v2-audit-certora-formal-verification; stellar.gitbook.io SCF handbook audit-bank pages; stellar.org/audit-bank/projects |
| Both public GitHub mirrors are STALE vs the freeze — **RESOLVED same day** | `Nectar-Network/nectar` last push 2026-07-25, `kunal-drall/nectar` 2026-08-09 — `audit-freeze-v1` + Session F/G artifacts existed only locally. Pushed 2026-08-16 after a two-pass full-history secrets sweep (CLEAN): `main` + `tranche-3` + tag live on both remotes, tag verified rendering | 2026-08-16 | `gh repo view` + push receipts this session; `docs/audit/READINESS-CHECK.md` |

## Live testnet surface at intake submission (2026-08-16, Session G/H boundary)

All verified 2026-08-16 by read-only `stellar contract invoke --send=no` sims,
independently reproduced by a second cold-client pass (operator-run).

| Claim | Value | Date | Source |
|---|---|---|---|
| Live pair on-chain state MATCHES the intake traction claims exactly | Vault `CDOGQY7NAE3BP4Q7RWBCBLW23Z36RNWNDNXX5DWNIEVMFEWP3GVEPXLR` `get_state` → `{total_usdc: 625960334, total_shares: 600000000, total_profit: 25960334, active_liq: 0}` → share price **1.0432672**, TVL $62.60. Registry `CD33A7IGNCOLVQ4EEINBVMVA7IHWXGN57R6YLE5AJEEKPA6VKC2E4IQD` `get_keeper(keeper-alpha GCC52N6U…)` → active, stake 1,000,000,000 (100 USDC), successful_fills 5, total_profit 25,960,334 (**== vault total_profit — internal consistency**), registered_at 1784809185 (≈Jul 23), last_draw_time 1786647844 (≈Aug 13), avg response ≈3.2 s (16174 ms / 5). Pair still runs the PRE-freeze wasm (redeploy = Session H) | 2026-08-16 | live sims this session + operator cold-client reproduction |
| **Root cause of the "TVL $0 / awaiting keeper API" frontend zeros (user-observed 2026-08-16)** | Production Vercel served the **Jul 25 build**, built before `NEXT_PUBLIC_API_URL` existed — `frontend/lib/api.ts:1` fallback `http://localhost:8080` was baked into that bundle, so every client fetch failed silently (fetchers catch-and-null). Fixed incidentally by the 2026-08-16 `main` pushes triggering fresh production deploys (READY ≈09:14 and 10:13 UTC, tip `f8f7744`). Verified post-fix from a cold client: both domains serve the identical bundle (etag `625c446a…`), zero `localhost:8080` refs across all chunks, `https://keeper-gamma-production.up.railway.app` + the live pair IDs baked in | 2026-08-16 | Vercel deployment list (`nectar-frontend`, prj_fqUPti9…); bundle greps both sessions |
| Frontend serving topology | ONE Vercel project (`nectar-frontend`) serves nectarnetwork.fun, testnet.nectar.monster, nectarn.fun, nectar.monster with the same deployment — domain-split per docs/NETWORKS.md is aspirational (mainnet post-audit), not current | 2026-08-16 | `get_project` domains list |
| keeper-gamma API: vault views correct, **Blend-derived views starved** | `/api/performance` serves the true vault numbers (CORS `*`); but the Blend event backfill fails on public-RPC rate limiting — the RPC returns a plain-text "no…" page and Go's JSON decoder surfaces `invalid character 'o' in literal null`. Consequence: `keeper_stats` knows only gamma at 0/0, `liquidations: null`, depositor list covers 500M of 600M shares, `pnl_pct` 0 despite +4.33%. Leaderboard is safe regardless — the page chunk reads `get_keepers`/`get_keeper` via `simulateTransaction` directly | 2026-08-16 | live API reads + chunk grep; operator reproduction |
| `/api/events` SSE sends 0 bytes to a fresh client (12 s window) | No backlog replay, no heartbeat — the "live" flag flips only on the first real broadcast (≤1 poll cycle when the keeper emits). Cosmetic | 2026-08-16 | live SSE probes (3 s + 12 s) |
| **Session H priority reorder (decision)** | H-top is now keeper RPC resilience: dedicated RPC provider and/or backoff on the Blend backfill + log the RAW response body on JSON decode failure (the 'o' bug took two sessions to diagnose from the error string alone). LoadPool reserve-list caching (formerly H1) moves second. Then: testnet redeploy of the frozen pair, keeper `add_profit` wiring, keeper-sdk interface adoption | 2026-08-16 | operator decision this session |
...
| **Session F / DECISION F-1 — add_profit WILL BE BUILT (option a): a registration-gated vault entry point for donated profit** | Explicit user choice, Session F pre-flight. Today the VLT-2 `NoDraw` guard means float-funded bad-debt profit accrues to the OPERATOR, not depositors (CORRECTION-REPORT limitation 2; "AMENDED 2026-08-11" row above). `add_profit(keeper, amount)`: `require_auth(keeper)`; the registry must verify the keeper (registered + active + stake > 0); `amount > 0`; USDC transferred in; `total_usdc += amount`; `total_profit += amount`; emits a DISTINCT `profit_added` event (donated profit must stay separable from draw-cycle profit in accounting); never touches draw accounting, cooldowns, or share issuance — it raises share price for existing holders only. Why (a) over de-scoping (b): VLT-1 virtual-offset shares already blunt the donation/inflation surface, auditors will examine donation paths regardless, and building it pre-freeze means the path gets AUDITED instead of bolted on after | 2026-08-16 | User decision recorded this session (Session F); `contracts/nectar-vault/src/lib.rs:307-315` @ `a0c6446` (NoDraw guard); docs/CORRECTION-REPORT.md limitation 2 |
| **Session F / DECISION F-2 — per-asset pause is CONTRACT-ENFORCED (option a): `draw(keeper, amount, asset)`** | Explicit user choice, Session F pre-flight. The draw declares the collateral asset it targets; the vault checks the global liquidation pause AND the per-asset pause on-chain, so honest keepers are hard-gated and the SCF T3 measurement ("per-asset and global pause mechanisms functional") is met contract-side; a signature change is cheap before the audit freeze and the keeper client is updated in the same session. LIMITATION (for the threat model): the asset is SELF-DECLARED — a malicious keeper can misdeclare to route around a per-asset pause; staking/slashing is the backstop for keeper misbehavior, and the global liquidation pause hard-blocks every draw regardless of declaration. Depositor exit is NOT conflated with the liquidation pause: deposit stays governed by the existing VLT-4 emergency pause, and withdraw must keep working during a liquidation pause (invariant pinned by test). Rejected (b) (keeper-enforced flags) because per-asset enforcement would then rest entirely on keeper honesty with no on-chain check | 2026-08-16 | User decision recorded this session (Session F); SCF T3 measurement wording (CLAUDE.md Tranche 3); `contracts/nectar-vault/src/lib.rs:233` @ `a0c6446` (pre-change draw signature) |
