...
| **RESOLVED (was OPEN) — USDC asset bridging on testnet**: a direct Soroswap pair Circle-USDC/Blend-USDC EXISTS on testnet (live `router_get_amounts_out` + `get_reserves` quotes, 2026-08-03): reserves ≈ 51.0 Circle / 49.0 Blend USDC, marginal rate ≈ 0.94–1.02, but only ~$50 depth per side, so any ≥~0.5 USDC conversion breaches a 1% par bound. Decision: **(b) DEX conversion layer, implemented with a par-anchored slippage guard** — `BLEND_POOLS=ADDR:active:POOL_USDC` enables vault-USDC↔pool-USDC conversion; the keeper live-quotes `router_get_amounts_in` before every draw and refuses off-parity routes (`ErrOffParity`) with zero capital moved, plus a 1-USDC canary check each scan cycle that suppresses task emission while the route is non-viable. TestnetV2 is configured active+convertible but will refuse conversions at current pair depth; the Nectar Sandbox pool (option (a), Task A4) settles Circle USDC directly — same-asset, no conversion | Route existence and depth are now measured facts, not assumptions; capital can never cross an off-par route | 2026-08-03 | `docs/evidence/a2-route-checks.json`; `keeper/dex/convert.go`; `keeper/dex/convert_test.go` |
